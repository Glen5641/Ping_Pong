...
/*
 * pingpong3.c -- pingpong program
 *
 * Send an array from process with rank 0 as broadcast
 * to rank 1:n and return all arrays to rank 0.
 *
 * Input: none.
...
int main (int argc, char* argv[]) { /* main */
  ...
  if (my_rank != master_rank) {
		...
		while(count <= 100) {
			...
			// Recieve the array
  	  mpi_error_code = MPI_Bcast(array, array_length, MPI_INT, tag, MPI_COMM_WORLD);
    ...
    // Grab rank 1
	  destination = 1;
	    ...
	    fprintf(stdout, "Ping-Pong: %d!\n", count);
			...
			//Collect all arrays from processes ! master
			int i = 1;
			while(i < number_of_processes){

				// Recieve the array
        mpi_error_code = MPI_Recv(array, array_length, MPI_INT, i, tag, MPI_COMM_WORLD, &recv_status);
				i++;
			}
		...
		fprintf(stdout, "Elapsed time of the program is %.10f\n", (time2 - time1));
  }
